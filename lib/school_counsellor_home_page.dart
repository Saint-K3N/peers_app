// lib/school_counsellor_home_page.dart
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:firebase_auth/firebase_auth.dart';
import 'package:flutter/material.dart';

class SchoolCounsellorHomePage extends StatelessWidget {
  const SchoolCounsellorHomePage({super.key});
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              // Greeting + stats
              if (uid.isEmpty)
                const _GreetingCardSC(
                  name: 'Counsellor',
                  counsellors: 0,
                  pendingApps: 0,
                )
              else
                _GreetingLoaderSC(userUid: uid),
              ...
              const SizedBox(height: 20),
              Text("Today's Schedule", style: t.titleMedium),
              const SizedBox(height: 12),

              if (uid.isEmpty)
                _emptyBox('Sign in to see today\'s schedule.')
              else
                _TodayScheduleListSC(),
            ],
          ),
        ),
      ),
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      children: [
        ...
        // avatar -> route to counsellor profile (optional)
        InkWell(
          borderRadius: BorderRadius.circular(30),
          onTap: () => Navigator.pushNamed(context, '/school-counsellors/profile'),
          child: CircleAvatar(
            radius: 18,
...
/* ----------------------- Greeting / Stats (Counsellor) ---------------------- */

class _GreetingLoaderSC extends StatelessWidget {
  final String userUid;
  const _GreetingLoaderSC({required this.userUid});
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      stream: usersCol.doc(userUid).snapshots(),
      builder: (context, userSnap) {
        final name = _pickName(userSnap.data?.data() ?? const {});
        // Approved counsellors & pending counsellor apps
...
class _GreetingCardSC extends StatelessWidget {
  final String name;
  final int counsellors;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text('Ready to oversee your counseling program?',
              style: t.bodyMedium?.copyWith(color: Colors.white.withOpacity(.9))),
          const SizedBox(height: 16),
          Row(
            children: [
              Expanded(child: _StatTile(value: '$counsellors', label: 'MyBuddies')),
              const SizedBox(width: 12),
    ...
    return Column(
      children: [
          ...
          children: [
            Expanded(
              child: _QuickActionTile(
                ...
                label: 'Scheduling',
                onTap: () => Navigator.pushNamed(context, '/school-counsellor/scheduling'),
              ),
            ),
            const SizedBox(width: 12),
            Expanded(
              child: _QuickActionTile(
                ...
                label: 'My Counsellors',
                onTap: () => Navigator.pushNamed(context, '/counsellor/my-counsellors'),
              ),
            ),
          ],
        ),
        ...
        _QuickActionTile(
          ...
          fullWidth: true,
          onTap: () => Navigator.pushNamed(context, '/counsellor/review-apps'),
        ),
      ],
    );
  }
...
class _TodayScheduleListSC extends StatelessWidget {
  const _TodayScheduleListSC();

  DateTime _startOfDay(DateTime d) => DateTime(d.year, d.month, d.day);
  DateTime _endOfDay(DateTime d) => DateTime(d.year, d.month, d.day, 23, 59, 59);

  Future<List<String>> _loadApprovedCounsellorIds() async {
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<List<String>>(
      future: _loadApprovedCounsellorIds(),
      builder: (context, cSnap) {
        final counsellorIds = cSnap.data ?? const <String>[];
        ...
        if (counsellorIds.isEmpty) {
          return SchoolCounsellorHomePage._emptyBox('No approved counsellors yet.');
        }
        ...
        return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
          ...
          builder: (context, snap) {
            ...
            if (docs.isEmpty) {
              return SchoolCounsellorHomePage._emptyBox("No appointments for today.");
            }

            return Column(
              children: [
                for (final d in docs) ...[
                  _SCScheduleTile(appDoc: d),
                  const SizedBox(height: 12),
                ]
              ],
            );
...
class _SCScheduleTile extends StatelessWidget {
  final QueryDocumentSnapshot<Map<String, dynamic>> appDoc;
  const _SCScheduleTile({required this.appDoc});
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, userSnap) {
        ...
        final title = 'Peer Meeting - $counsellorName';
              ...
              Row(
                ...
                children: [
                  Container(
                    ...
                    alignment: Alignment.center,
                    child: const Icon(Icons.psychology_alt_outlined, color: Colors.black87, size: 24),
                  ),
                  const SizedBox(width: 12),
                  Expanded(
                    child: Column(
