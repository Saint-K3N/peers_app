// lib/peer_counsellor_profile_page.dart
...
class PeerCounsellorProfilePage extends StatefulWidget {
  const PeerCounsellorProfilePage({super.key});

  @override
  State<PeerCounsellorProfilePage> createState() =>
      _PeerCounsellorProfilePageState();
}

class _PeerCounsellorProfilePageState
    extends State<PeerCounsellorProfilePage> {
  ...
  Future<void> _pickAndUploadAvatar() async {
    ...
    try {
      ...
      if (kIsWeb) {
        ...
        task = ref.putData(
            bytes, SettableMetadata(contentType: _guessMime(filename)));
      } else {
        final fp = file.path;
        if (fp == null || fp.isEmpty) throw 'Invalid file path.';
        task = ref.putFile(
            File(fp), SettableMetadata(contentType: _guessMime(filename)));
      }

      task.snapshotEvents.listen((snap) {
        if (snap.totalBytes > 0) {
          setState(
                  () => _avatarProgress = snap.bytesTransferred / snap.totalBytes);
        }
      });
      ...
      await _usersCol.doc(_uid).set({kFieldPhotoUrl: url},
          SetOptions(merge: true));
      try {
          ...
          builder: (context, userSnap) {
            ...
            return SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  _HeaderCounsellor(onLogout: () => _logout(context)),
                  ...
                  /* ----------- Past Counselling Sessions (LIVE) ----------- */
                  _PastSessionsCardCounsellor(helperId: _uid),
                  ...
                  /* ------------------------- My Peers (LIVE) ------------------------ */
                  _PeersYouWorkedWith(helperId: _uid),
                ],
              ),
            );
          },
...
class _HeaderCounsellor extends StatelessWidget {
  final VoidCallback onLogout;
  const _HeaderCounsellor({required this.onLogout});
    ...
    final avatar = ClipRRect(
      ...
      child: Container(
        ...
        child: photoUrl.isNotEmpty
          ...
          child: Text(
            _initials(
                fullNameFallback.isNotEmpty ? fullNameFallback : 'C'),
...
/* -------- Past Counselling Sessions (helperId = me, LIVE) -------- */

class _PastSessionsCardCounsellor extends StatelessWidget {
  final String helperId;
  const _PastSessionsCardCounsellor({required this.helperId});
  ...
  Future<void> _deleteAllCompleted(BuildContext context) async {
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text(
            'This will permanently remove all your completed counselling sessions.'),
        actions: [
          ...
          return Column(
            ...
            children: [
              Row(
                children: [
                  Expanded(
                    child: Text('Past Counselling Sessions ($total)',
                        style: t.titleMedium
                  ...
                  FilledButton(
                    ...
                    onPressed:
                    docs.isEmpty ? null : () => _deleteAllCompleted(context),
              ...
              if (docs.isEmpty)
                ...
                Column(
                  children: [
                    for (final d in docs) ...[
                      _PastCounsellorSessionRow(appDoc: d),
...
class _PastCounsellorSessionRow extends StatelessWidget {
  final QueryDocumentSnapshot<Map<String, dynamic>> appDoc;
  const _PastCounsellorSessionRow({required this.appDoc});
...
/* ----------------------- Peers you worked with (LIVE) --------------------- */

class _PeersYouWorkedWith extends StatelessWidget {
  final String helperId;
  ...
  Future<List<_PeerMini>> _loadPeers() async {
    ...
    peers.sort((a, b) => b.sessionsWithMe.compareTo(a.sessionsWithMe));
    return peers;
  }

  void _openPeerDetail(BuildContext context, _PeerMini p) {
    // ✅ UNCHANGED: Navigate to the peer counsellor's specific peer detail page
    Navigator.pushNamed(
      context,
  ...
  Future<void> _reportPeer(BuildContext context, _PeerMini peer) async {
    ...
    final result = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Please provide a reason for reporting this peer:'),
            const SizedBox(height: 12),
            TextField(
    ...
    if (result == true && context.mounted) {
      try {
        await FirebaseFirestore.instance.collection('reports').add({
          'reportedUserId': peer.peerId,
          'reportedByUserId': currentUser.uid,
          'reportedByRole': 'peer_counsellor',
          'reportedUserRole': 'student',
          'reportedUserName': peer.name,
          'reportedByName': currentUser.displayName ?? 'Peer Counsellor',
  ...
  Widget build(BuildContext context) {
    final t = Theme.of(context).textTheme;
    return FutureBuilder<List<_PeerMini>>(
      future: _loadPeers(),
      builder: (context, snap) {
        final peers = snap.data ?? const <_PeerMini>[];
                                ...
                                children: [
                                  Text(p.name,
                                      style: t.titleMedium?.copyWith(
                                          fontWeight: FontWeight.w700)),
                                  const SizedBox(height: 4),
                                  Text('${p.sessionsWithMe} completed session${p.sessionsWithMe != 1 ? "s" : ""}',
                                      style: t.bodySmall
                                  ...
                                  Row(
                                    children: [
                                      Expanded(
                                        child: SizedBox(
                                          ...
                                          child: OutlinedButton(
                                            onPressed: () => _openPeerDetail(context, p),
                                      ...
                                      SizedBox(
                                        height: 32,
                                        child: OutlinedButton(
                                          onPressed: () => _reportPeer(context, p),
                                          style: OutlinedButton.styleFrom(
                                            side: const BorderSide(
                                                color: Colors.red),
                                            shape: RoundedRectangleBorder(
