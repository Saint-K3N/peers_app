// lib/school_counsellor_scheduling_page.dart
...
class SchoolCounsellorSchedulingPage extends StatefulWidget {
  ...
  @override
  State<SchoolCounsellorSchedulingPage> createState() => _SchoolCounsellorSchedulingPageState();
}

class _SchoolCounsellorSchedulingPageState extends State<SchoolCounsellorSchedulingPage> {
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              _CalendarArea(
                scUid: _uid, // <- filter by schoolCounsellorId (current user)
              ...
              _DayTasksList(
                scUid: _uid, // <- filter by schoolCounsellorId
...
class _SCHeaderBar extends StatelessWidget {
  const _SCHeaderBar();
...
class _CalendarArea extends StatelessWidget {
  final String scUid; // filter by schoolCounsellorId
  ...
  @override
  Widget build(BuildContext context) {
    // Appointments created by / owned by this School Counsellor
    final appsStream = FirebaseFirestore.instance
        .collection('appointments')
        .where('schoolCounsellorId', isEqualTo: scUid)
        .snapshots();

    // Optional personal events for the counsellor (change the collection to your schema if needed)
    final eventsStream = FirebaseFirestore.instance
        .collection('school_counsellor_events')
        .where('schoolCounsellorId', isEqualTo: scUid)
        .snapshots();
    ...
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, appsSnap) {
        return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
          stream: eventsStream,
          builder: (context, evSnap) {
            // Hide progress bar per your request — render nothing while loading
...
class _TaskItem {
  ...
  factory _TaskItem.appointment(String id, Map<String,dynamic> m) {
    ...
    final helperId = (m['helperId'] ?? '').toString();
    final helperName = (m['helperName'] ?? '').toString();
...
class _TaskCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Title:
    // - Appointment: "Counsellor Meeting – {name}"
    // - Event: item.title
    Widget titleWidget;
    if (isAppt) {
      if (item.helperName.isNotEmpty) {
        titleWidget = Text('Counsellor Meeting – ${item.helperName}',
        ...
        titleWidget = FutureBuilder<DocumentSnapshot<Map<String, dynamic>>>(
          ...
          builder: (context, snap) {
            ...
            if (name.isEmpty) name = 'Counsellor';
            return Text('Counsellor Meeting – $name',
                style: t.titleMedium?.copyWith(fontWeight: FontWeight.w700));
          },
        );
      } else {
        titleWidget = Text('Counsellor Meeting – Counsellor',
            style: t.titleMedium?.copyWith(fontWeight: FontWeight.w700));
      }
    } else {
    ...
    void _openDetails() {
      if (isAppt) {
        Navigator.pushNamed(
          context,
          '/school-counsellors/booking-info', // TODO: adjust to your actual route name
