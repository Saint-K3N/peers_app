// lib/hop_tutor_detail_page.dart
import 'dart:math' as math;
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:flutter/material.dart';

/* =========================== HOP Tutor Detail Page =========================== */

class HopTutorDetailPage extends StatefulWidget {
  const HopTutorDetailPage({super.key});

  @override
  State<HopTutorDetailPage> createState() => _HopTutorDetailPageState();
}

class _HopTutorDetailPageState extends State<HopTutorDetailPage> {
  bool _tabSessions = true; // Sessions | Students

  @override
  Widget build(BuildContext context) {
    final args = (ModalRoute.of(context)?.settings.arguments as Map?) ?? {};
    final tutorId = (args['tutorId'] as String?) ?? '';
    final t = Theme.of(context).textTheme;

    if (tutorId.isEmpty) {
      return Scaffold(
        body: SafeArea(
          child: Center(child: Text('Missing tutorId.', style: t.bodyMedium)),
        ),
      );
    }
    ...
    return Scaffold(
      body: SafeArea(
        child: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
          stream: usersCol.doc(tutorId).snapshots(),
            ...
            return SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (_tabSessions)
                    _TutorSessionsList(tutorId: tutorId, tutorName: name)
                  else
                    _TutorStudentsList(tutorId: tutorId),
                ],
              ),
            );
          },
...
// Extract a student id safely from many shapes. Returns null if there’s no real student.
...
class _DetailHeaderCard extends StatelessWidget {
  final String tutorId, name, email;
  ...
  Future<String> _lastSession(String tutorId) async {
        ...
        return Container(
          ...
          child: Column(
            children: [
              ...
              Row(
                children: [
                  // STUDENTS (distinct studentIds; ignore nulls / HOP-only)
                  _MiniStatBox(
                    title: 'Students',
...
class _TutorSessionsList extends StatelessWidget {
  final String tutorId;
  final String tutorName;
  const _TutorSessionsList({required this.tutorId, required this.tutorName});
    ...
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snap) {
        ...
        return Column(
          children: [
            for (final d in docs) ...[
              _SessionRow(m: d.data(), tutorName: tutorName),
              const SizedBox(height: 10),
            ],
          ],
        );
...
class _SessionRow extends StatelessWidget {
  ...
  final String tutorName;
  const _SessionRow({required this.m, required this.tutorName});
      ...
      child: Row(
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
                color: bg, borderRadius: BorderRadius.circular(8), border: Border.all(color: fg)),
...
class _TutorStudentsList extends StatelessWidget {
  final String tutorId;
  const _TutorStudentsList({required this.tutorId});
