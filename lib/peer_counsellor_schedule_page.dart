...
class _PeerCounsellorSchedulePageState extends State<PeerCounsellorSchedulePage> {
  ...
  Future<void> _reschedule(BuildContext context, String apptId, Map<String,dynamic> m) async {
    ...
    // This logic is for the CANCEL action, but since Reschedule policy
    // is also complex, we will handle the complex logic in the dialog.
    // For now, keep the policy gate for > 24h away appointments.
    final isWithin24Hours = start.difference(now).abs() <= const Duration(hours: 24);
    final isConfirmed = (m['status'] ?? '').toString().toLowerCase() == 'confirmed';

    // For a CONFIRMED appointment <= 24h away, we allow reschedule
    // but we need a reason and set status to pending_reschedule
    if (isConfirmed && isWithin24Hours) {
      // Allow through and handle the reason/status logic in the dialog/post-dialog logic below
    } else if (start.difference(now) < const Duration(hours: 24)) {
      if (mounted) {
    ...
    final reasonCtrl = TextEditingController(text: isConfirmed && isWithin24Hours ? '' : 'Reschedule needed');
            ...
            content: Column(
              ...
              children: [
                ...
                if (isConfirmed && isWithin24Hours) ...[
                  ...
                  Text('Reason for reschedule (max 20 chars):', style: Theme.of(context).textTheme.bodySmall),
                  TextField(
                    controller: reasonCtrl,
                    maxLength: 20,
                    decoration: const InputDecoration(hintText: 'Required', border: OutlineInputBorder()),
                  ),
                ]
              ],
            ),
    ...
    if (isConfirmed && isWithin24Hours && reason.isEmpty) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Reschedule reason is required.')));
      }
      return;
    }
    ...
    // Set status to pending_reschedule if cancelled/rescheduled from inside 24h window
    final status = (isConfirmed && isWithin24Hours) ? 'pending_reschedule' : 'confirmed';
    ...
    await FirebaseFirestore.instance.collection('appointments').doc(apptId).update(updates);

    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(status == 'pending_reschedule' ? 'Rescheduled. Student needs to confirm.' : 'Rescheduled.'))
      );
    }
  }
          ...
          child: Column(
            ...
            children: [
              ...
              _DayTasksList(
                ...
                // Cancel: policy-gated (fixed reason if pending <= 24h, custom reason if confirmed <= 24h, no reason if > 24h)
                onCancel: (id) async {
                  final doc = await FirebaseFirestore.instance.collection('appointments').doc(id).get();
                  final m = doc.data() ?? {};
                  final ts = m['startAt'];
                  ...
                  final remainingTime = start.difference(DateTime.now());
                  final isWithin24Hours = remainingTime <= const Duration(hours: 24);
                  ...
                  if (isWithin24Hours) {
                    if (status == 'pending') {
                      reason = 'Last minute appointment, unable to make it';
                      needsReason = true;
                    ...
                    if (start.difference(DateTime.now()) < const Duration(hours: 24)) {
                      // Should be blocked by canModify, but double check
                      if (mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                  ...
                  if (requireInput) {
                    ...
                    final ok = await showDialog<bool>(
                      ...
                      builder: (_) => AlertDialog(
                        ...
                        content: Column(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            const Text('Please provide a reason (max 20 chars).'),
                            const SizedBox(height: 12),
                            TextField(
                              controller: reasonCtrl,
                              maxLength: 20,
                    ...
                    if (ok == true) {
                      final input = reasonCtrl.text.trim();
                      if (input.isEmpty) {
                        if (mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Cancellation reason is required.')));
                        }
                        return;
                      }
                      reason = input;
                    ...
                    final ok = await showDialog<bool>(
                      context: context,
                      builder: (_) => AlertDialog(
                        title: const Text('Cancel Pending Appointment?'),
                        content: const Text('You are cancelling a last-minute appointment. The reason "Last minute appointment, unable to make it" will be recorded.'),
                        actions: [
                          TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Close')),
                          FilledButton(onPressed: () => Navigator.pop(context, true), child: const Text('Cancel')),
                        ],
...
class _TaskItem {
  ...
  factory _TaskItem.appointment(String id, Map<String,dynamic> m) {
    ...
    final schoolCounsellorId = (m['schoolCounsellorId'] ?? '').toString().trim();
    final studentId = (m['studentId'] ?? '').toString().trim();

    // Determine which type of appointment this is
    final isSchoolCounsellorAppointment = schoolCounsellorId.isNotEmpty;

    // Use the appropriate ID and name based on appointment type
    final personId = isSchoolCounsellorAppointment ? schoolCounsellorId : studentId;
    final personName = isSchoolCounsellorAppointment
        ? (m['schoolCounsellorName'] ?? '').toString()
        : (m['studentName'] ?? '').toString();

    final isCounselling = (m['helperRole'] ?? '').toString().toLowerCase().contains('counsellor');

    // Title logic:
    // - If School Counsellor appointment: "Counselling with School Counsellor"
    // - If Student appointment: "Counselling Session" (existing behavior)
    return _TaskItem._(
      id: id,
      type: 'appointment',
      data: m,
      ...
      // FIXED: Base title without "with" - will be added in the widget
      title: isCounselling
          ? (isSchoolCounsellorAppointment ? 'Counselling (SC)' : 'Counselling Session')
          : 'Session',
      status: status,
      venue: venue,
...
class _TaskCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Widget titleWidget;
    if (isAppt) {
      // Check if this is a School Counsellor appointment
      final schoolCounsellorId = (item.data['schoolCounsellorId'] ?? '').toString().trim();
      final isSchoolCounsellorAppointment = schoolCounsellorId.isNotEmpty;

      // Label for FutureBuilder fallback
      final label = isSchoolCounsellorAppointment ? 'School Counsellor' : 'Student';

      if (item.studentName.isNotEmpty) {
        // item.studentName contains either the School Counsellor name or Student name
        // item.title is already "Counselling with School Counsellor" or "Counselling Session"
        titleWidget = Text(
            isSchoolCounsellorAppointment
                ? 'Counselling with School Counsellor — ${item.studentName}'
                : '${item.title} with ${item.studentName}',
        ...
        titleWidget = FutureBuilder<DocumentSnapshot<Map<String, dynamic>>>(
          ...
          builder: (context, snap) {
            ...
            if (name.isEmpty) name = label;  // Fallback: "School Counsellor" or "Student"

            return Text(
                isSchoolCounsellorAppointment
                    ? 'Counselling with School Counsellor — $name'
                    : '${item.title} with $name',
                style: t.titleMedium?.copyWith(fontWeight: FontWeight.w700)
            );
          },
        );
